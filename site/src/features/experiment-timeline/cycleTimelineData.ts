...
export const cycleStages: CycleStage[] = [
  ...
  },
  {
    ...
    generic: { zh: '核自旋计算基内的单比特原语是 RF 旋转 Rφ(θ)。相互作用门则先把选定计算态耦合到 Rydberg 态，再用阻塞实现条件相位；读出 Raman 不是单比特计算门。', en: 'The one-qubit primitive within the nuclear-spin basis is an RF rotation Rφ(θ). An interaction gate couples a selected computational state to a Rydberg state and uses blockade for a conditional phase; the readout Raman path is not a one-qubit computational gate.' },
