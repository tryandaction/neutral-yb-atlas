...
export const ytterbiumChapter: Chapter = {
  ...
  sections: [
    ...
    },
    {
      ...
      answer: { zh: '选择分两步：Yb 的两个 J=0 流形使核自旋信息可与冷却、读出和 Rydberg 门接口分工；再在 Yb 同位素中选择 I=1/2 的 171Yb，使 J=0 流形恰有 mF=±1/2 这一个最小核自旋双态。', en: 'The choice has two steps: Yb offers two J=0 manifolds that separate nuclear-spin information from cooling, readout and Rydberg-gate interfaces; among Yb isotopes, I=1/2 in 171Yb leaves exactly the minimal mF=±1/2 nuclear-spin doublet in a J=0 manifold.' },
      reasoning: [
        { zh: 'Yb 的 6s² 1S0 和 6s6p 3P0 流形均有 J=0。理想的 J=0 态没有一阶电子 Zeeman 磁矩；残余磁敏感性来自核自旋以及实际偏置场、光移和技术噪声。这个结论并不等于 Rb、Cs 不能使用磁不敏感钟比特，而是 Yb 提供了不同电子流形之间的职责分离。', en: 'Both 6s² 1S0 and 6s6p 3P0 in Yb have J=0. An ideal J=0 state has no first-order electronic Zeeman moment; residual sensitivity comes from nuclear spin, bias fields, light shifts and technical noise. This does not mean Rb or Cs cannot use magnetically insensitive clock qubits; it identifies Yb as an architecture with duties separated across electronic manifolds.' },
        { zh: '偶 A Yb 同位素有 I=0，因而 J=0 流形中只有 mF=0，不能直接提供核自旋双态。173Yb 的 I=5/2 提供六个投影并可用于 qudit，但会增加光抽运、谱选择和旁观态控制；171Yb 的 I=1/2 则恰好只保留两个投影。', en: 'Even-A Yb isotopes have I=0, so a J=0 manifold contains only mF=0 and does not directly supply a nuclear-spin doublet. I=5/2 in 173Yb supplies six projections and can support qudits, but increases optical pumping, spectral selection and spectator-state control; I=1/2 in 171Yb leaves exactly two projections.' },
        { zh: '因此可把计算链明确写成：J=0 核自旋存储 → 钟跃迁映射到 3P0 → 选择性 Rydberg 激发产生 B|rr⟩⟨rr| → 返回计算流形并测量。强相互作用不必长期存在；它只在双比特门所需的时间窗口开启。', en: 'The computation chain can therefore be written explicitly: J=0 nuclear-spin storage → clock transfer to 3P0 → selective Rydberg excitation yielding B|rr⟩⟨rr| → return to a computational manifold and measurement. Strong interaction need not persist; it is switched on only during the two-qubit gate window.' },
